...
exports.get_receiving_product = function(conn, type, ids) {
    ...
    conn.getConnection(function(err, connection){
		if (type == 1)
			var query_str = 'SELECT a.rqty,a.rtype,b.*,c.cname FROM receiving_item_tab a INNER JOIN products_tab b ON a.rpid=b.pid JOIN categories_tab c ON b.pcid=c.cid WHERE a.riid='+ids+' AND a.rstatus=1 AND b.pstatus=1 ORDER BY b.pid DESC';
		else
...
exports.get_receiving_detail_approved = function(conn, type, id) {
    ...
    conn.getConnection(function(err, connection){
		if (type == 1)
			...
			var query_str = 'SELECT a.rqty,a.rtype,b.*,c.cname FROM receiving_item_tab a INNER JOIN products_tab b ON a.rpid=b.pid JOIN categories_tab c ON b.pcid=c.cid WHERE a.riid = ? AND a.rstatus=1 AND b.pstatus=1 ORDER BY b.pid DESC';
