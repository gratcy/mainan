exports.exec_transaction = function(req, res) {
	...
	if (!str) {
		...
		var data = [];
		models.getOrder(req,input).then(function(OrderData) {
			models.getReceiving(req,input).then(function(ReceiveData) {
				...
				if (input.type && input.type.length > 0) {
						for(var i=0;i<input.type.length;++i) {
							...
							if (!isNaN(index)) {
								if (index == 3) {
									data = _.union(OrderData,ReceiveData);
									is_Receive = true;
									break;
								}
							}
				...
				if (rtype == 1) {
					...
					Object.keys(foo).map(function(objectKey, index) {
						...
						for(var i=0;i<foo[objectKey].length;++i) {
							...
							as.push({no:(i+1)+'.',ttype:bam.ttype,uid:bam.uid,cname:bam.cname,uemail:bam.uemail,netto:netto,bruto:bruto,tprice:price,tpricebase:pricebase,tdate:bam.tdate,tcreatedby:bam.tcreatedby,tno:bam.tno,tdesc:bam.tdesc,tqty:bam.tqty,tammount:bam.tammount,tdiscount:discount,ttotal:bam.ttotal,pname:bam.pname});
						}
						gTrans.push(as);
					});
					rdata = gTrans;
					...
					Object.keys(foo).map(function(objectKey, index) {
						...
						for(var i=0;i<foo[objectKey].length;++i) {
							...
							as.push({no:(i+1)+'.',uid:bam.uid,uemail:bam.uemail,cname:bam.cname,netto:netto,bruto:bruto,tprice:price,tpricebase:pricebase,tdate:bam.tdate,tcreatedby:bam.tcreatedby,tno:bam.tno,tdesc:bam.tdesc,tqty:bam.tqty,tammount:bam.tammount,tdiscount:discount,ttotal:bam.ttotal,pname:bam.pname});
						}
						guser.push(as);
					});
					rdata = guser;
